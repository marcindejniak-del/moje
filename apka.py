...
class AnalizaInercjiExcelApp:
    ...
    def __init__(self, root):
        ...
        self.root.title('Analiza inercji – nowoczesny interfejs  v2.0')
    ...
    def _bind_keyboard_shortcuts(self):
        self.root.bind('<Control-e>', lambda e: self.eksportuj_do_excel())
        self.root.bind('<Control-E>', lambda e: self.eksportuj_do_excel())
        self.root.bind('<F5>', lambda e: self.oblicz_inercje_wszystkie())
        self.root.bind('<Control-s>', lambda e: self.zapisz_sesje())
        self.root.bind('<Control-S>', lambda e: self.zapisz_sesje())
        self.root.bind('<Control-r>', lambda e: self.przelicz_aktywny_model())
        self.root.bind('<Control-R>', lambda e: self.przelicz_aktywny_model())
    ...
    def aktualizuj_status(self, tekst):
        try:
            ...
            self.label_status.config(text=f'[{ts}] {tekst}')
    ...
    def utworz_interfejs(self):
        ...
        self.label_subtitle = tk.Label(header, bg=pal['header'], fg=pal['muted'], font=self.font_subtitle, text='Import EPC + DYD, analiza E_sys oraz zapotrzebowania mocy z sekcji LOAD DATA    |    Skróty: F5 = Przelicz  Ctrl+E = Eksport Excel  Ctrl+S = Zapisz sesję  Ctrl+R = Przelicz model')
    ...
    def eksportuj_do_excel(self):
        ...
        try:
            ...
            if hourly_rows:
                ...
                ws_hours.auto_filter.ref = f'A1:L{len(hourly_rows) + 1}'
